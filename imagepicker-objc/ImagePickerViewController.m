    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (errorObj) {
            ...
            if (numLabels > 0) {
                ...
                AVSpeechUtterance *utterance = [AVSpeechUtterance speechUtteranceWithString:[@"You are currently looking at " stringByAppendingString:labelResultsText]];
                synthesizer.delegate = self;
                [utterance setRate:0.35f];
                [synthesizer speakUtterance:utterance];
                self.labelResults.text = labelResultsText;
            } else {
                self.labelResults.text = @"No labels found";
                [self captureStillImageAndAnalyze];
            }
        }
    });
...
- (void)speechSynthesizer:(AVSpeechSynthesizer *)synthesizer
 didFinishSpeechUtterance:(AVSpeechUtterance *)utterance {
    [self performSelector:@selector(captureStillImageAndAnalyze) withObject:Nil afterDelay:2];
}
...
- (void)viewDidLoad {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self performSelector:@selector(captureStillImageAndAnalyze) withObject:Nil afterDelay:3];
    });
    
}

-(void)captureStillImageAndAnalyze {
    ...
    [stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection
                                                         completionHandler:^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
                                                             ...
                                                             NSString *image64 = [self base64EncodeImage:image];
